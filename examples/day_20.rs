...
fn solve(input: &str) -> i64 {
    ...
    for path_point in shortest_path_positions.iter() {
        for direction in all_directions() {
            if let Some(first_blocked) = try_move(&map, *path_point, direction) {
                if is_blocked(&map, first_blocked) {
                    for second_direction in all_directions() {
                        if let Some(point_after_block) = try_move(&map, first_blocked, second_direction) {
                            if point_after_block != *path_point && shortest_path_positions.contains(&point_after_block) {
                                shortcuts.insert((*path_point,  point_after_block, 2));
                            }
                        }
                    }
                }
    ...
    for (index, (shortcut_start, shortcut_end, shortcut_cost)) in shortcuts.iter().enumerate() {
        println!("Analysing {index}/{}", shortcuts.len());
